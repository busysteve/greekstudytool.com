...
function playSound5(path, subpath, soundfile ) {
	...
	//var htmld = "<audio autoplay id='sound_player5'><source src=\""+fullpath+"\" type=\"audio/mpeg\" width=\"0\" height=\"0\">Your browser currently does not support this audio.</audio>";
...
{
    var audiodummy = document.getElementById("sound_player5");
...
function displayText( book, chapter)
{
	...
	document.getElementById('followalong').innerHTML='<br><i onclick=" verse_read = 0; followAlongReading(0);">( βετα! ) Follow-Along-Reading</i>';
...
function doStrongsTitle( snum )
{
	var str = lookUpStrongs( snum ); 
	//val.setAttribute('title', word );
	val.setAttribute('title', str );
}

function doStrongsWord( snum )
{
	return lookUpStrongs( snum ); 
	//val.setAttribute('title', word );
}
...
function lookUpStrongs( snum )
{

    var num = snum.split("G")[1];
    showparselink = 0;

	if( showparselink == 1 )
		return "<i onclick='clearTip(this.parentNode);'>close</i><br/>" + "WORD1" + " | " + "WORD2" + " | <i onclick='showparse(this, \"" + "WORD3" + "\");'>"+ "WORD4" +"</i> | " + "\n" + strongs_list[num];
	else
		return strongs_list[num];
}
...
function toggleBG()
{
	...
	setCookie('lxx_background', g_background, 365 );
