...
function toggleEnglishInline()
{
	...
	setCookie('eng_per_line', g_eng_per_line, 365 );
...
function displayText( book, chapter)
{
	...
	g_eng_per_line = getCookie( 'eng_per_line' );
	...
	for( i=0; i < grk_text[chp].length; i++ )
	{
		...
		if( g_eng_per_line == 1 )
		{	
			document.getElementById('greektext').innerHTML += "<br>" + webster[book][ch][i][0];
			document.getElementById('greektext').innerHTML += "<br><br>";
		}
		if( g_eng_per_line == 0 && g_verse_per_line == 1 )
