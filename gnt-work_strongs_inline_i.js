...
function playSound5(path, subpath, soundfile ) {
	var fullpath = path+subpath+soundfile;

	//var htmld = "<audio controls autoplay id='sound_player5'><source src=\""+fullpath+"\" type=\"audio/mpeg\" width=\"300\" height=\"100\">Your browser currently does not support this audio.</audio>";
	var htmld = "<source src=\""+fullpath+"\" type=\"audio/mpeg\" width=\"0\" height=\"0\">Your browser currently does not support this audio.";
	
	var dummy = document.getElementById("sound_player5");
	
	dummy.innerHTML = htmld;
	
    //dummy.stop();

	var playPromise = dummy.load();
...
function playSoundSWo(path, subpath, soundfile ) {
...
// document.getElementById("dummy").play();
 ...
 var playPromise = document.getElementById("mysound").load();
/*
 if (playPromise !== undefined) {
    playPromise.then(_ => {
      // Automatic playback started!
...
*/
 ...
 playPromise = document.getElementById("mysound").play();
/*
  if (playPromise !== undefined) {
    playPromise.then(_ => {
      // Automatic playback started!
...
function playSound(path, subpath, soundfile ) {
	...
	var htmld = "<audio id='sound_player'><source src=\""+fullpath+"\" type=\"audio/mpeg\" width=\"0\" height=\"0\">Your browser currently does not support this audio.</audio>";
...
var verseLinks = null;
var verse_read = 0;
var keep_reading = false;
//var audiodummy = null;

function followAlongReadingHandler()
{
    audiodummy = document.getElementById("sound_player5");

    audiodummy.controls = true;
    
    //var audiodummy = document.getElementById("mysound");

    
    var ended_func = function() {
        ++verse_read;
        if( keep_reading == true && verse_read < verseLinks.length )
                followAlongReading( verse_read );
        else
            keep_reading = false;
    };


    audiodummy.addEventListener( 'ended', function(){ setTimeout( ended_func, 80 ) } );
}

function followAlongReading(ver)
{
    verseLinks = document.getElementsByClassName('vnum');
    ...
    verseLinks[ver].click();
    keep_reading = true;
    
    //var audiodummy = sw_player ? document.getElementById("mysound") : document.getElementById("dummy");

    console.log( 'Follow-along verse(' + ver + ')' );

}

function getCookie_org(c_name)
{
...
function setCookie_org(c_name,value,expiredays)
{
	var exdate=new Date();
...
{
    playSound5();
    verse_read = 0;  // Reset Audio Follow Along
	...
	document.getElementById('followalong').innerHTML='<br><i onclick=" verse_read = 0; followAlongReading(0); document.getElementById(\'sound_player5\').controls=true; pageScrollRefresh();">Follow-Along-Reading</i>';
	...
	for( i=0; i < grk_text[chp].length; i++ )
	{
	
		{
			...
			{
				inpart += "<b class='vnum' style=\"color: #f04040;\" onClick=\"highlight( '" + book_index[book][0]+" "+ch+":"+grk_text[chp][i][1]+"'); " + player + "('" + audio_path + "', '" + book_index[g_last_book][2] + "', '" + grk_text[chp][i][0] + "');\">"+grk_text[chp][i][1]+"</b>";
		...
		{	
            ...
            {
                inpart += "<br><version style='font-size: 12px;'>Webster</version> &nbsp;" + "Failed to load Webster verse";
            }
		}
		...
		{	
            ...
            {
                inpart += "<br><version style='font-size: 12px;'>MLV</version> &nbsp;" + "Failed to load MLV verse";
            }
		}
...
function lookUpStrongs( ch, chp, vers, word, showparselink )
{
    ...
    if( eval( strongs[chp][vers] ) == 'undefined' )
...
function placeDiv() 
{
  var d = document.getElementById('audioDiv');
  var bod = document.getElementById('bodyElement');
  
  rect = d.getBoundingClientRect();
  rectBody = bod.getBoundingClientRect();
  
  ydiff = rect.bottom - window.innerHeight;
  
  d.style.position = "absolute";
  d.style.left = 0+'px';
  d.style.top = ((-rectBody.top)+window.innerHeight-50)+'px';
}
...
function pageScroll(e) {
	...
	if( ( posfactor = isInViewport2(e) ) != 0 )
	{
		window.scrollBy(0,10*posfactor);
		scrolldelay = setTimeout(pageScroll(e),50);
	}
}

function pageScrollRefresh() {
	window.scrollBy(0,1);
}
