...
function playSound(path, subpath, soundfile ) {
	...
	var htmld = "<audio autoplay id='sound_player'><source src=\""+fullpath+"\" type=\"audio/mpeg\" width=\"0\" height=\"0\">Your browser currently does not support this audio.</audio>";
...
function followAlongReading(ver)
{
    ...
    var audiodummy = sw_player ? document.getElementById("mysound") : document.getElementById("sound_player");
    //var audiodummy = document.getElementById("mysound");
    
    audiodummy.addEventListener( 'ended', function() {
        if( (ver++) < verseLinks.length )
            followAlongReading( ver );
    } );
...
function displayText( book, chapter)
{
	...
	document.getElementById('followalong').innerHTML='<br><i onclick="followAlongReading(0);">(NEW!) Follow-Along-Reading</i>';
