...
function playSoundMain(path, subpath, soundfile ) {
   ...
   //swfobject.embedSWF("player_mp3_mini.swf", soundfile, "200", "20", "9.0.0", "", {"mp3":"./vocab/"+soundfile}, {"bgcolor":"#085c68"} );
	...
	var htmld = "<source src=\"http://greekstudytool.com/"+fullpath+"\" type=\"audio/mpeg\" width=\"0\" height=\"0\"><object class=\"playerpreview\" type=\"application/x-shockwave-flash\" data=\"./player_mp3_mini.swf\"  width=\"0\" height=\"0\" ><param name=\"FlashVars\" value=\""+fullpath+"\"/><embed href=\"./player_mp3_mini.swf\" width=\"0\" height=\"0\" flashvars=\""+fullpath+"\" /></object></source>";
...
function playSoundSW(path, subpath, soundfile ) {
 ...
 "<source src=\"http://greekstudytool.com/"+fullpath+"\" type=\"audio/mpeg\"><object class=\"playerpreview\" type=\"application/x-shockwave-flash\" data=\"player_mp3_mini.swf\"  width=\"0\" height=\"0\" ><param name=\"FlashVars\" value=\"mp3="+fullpath+"\"/><embed href=\"player_mp3_mini.swf\" width=\"0\" height=\"0\" flashvars=\"mp3="+soundfile+"\" /></object>";
...
function displayText( book, chapter)
{
	...
	g_background = getCookie( 'background' ) == null ? 1 : getCookie( 'background' );
...
function lookUpStrongs( ch, chp, vers, word, showparselink )
{
	...
	{
		if( i >= ln ) break; 
		if( compareWords( strongs[chp][vers][i][0], wrdr) <= 1 )
			if( showparselink == 1 )
				return "<i onclick='clearTip(this.parentNode);'>close</i><br/>" + strongs[chp][vers][i][0] + " | " + strongs[chp][vers][i][1] + " | <i onclick='showparse(this, \"" + strongs[chp][vers][i][2] + "\");'>"+ strongs[chp][vers][i][2] +"</i> | " + "\n" + strongs_list[strongs[chp][vers][i][1]];
			else
				return "\n" + strongs[chp][vers][i] + "\n" + strongs_list[strongs[chp][vers][i][1]];
