...
function displayText( book, chapter)
{
	...
	for( i=0; i < grk_text[chp].length; i++ )
	{
		...
		if( g_eng_web_per_line == 1 )
		{	
			document.getElementById('greektext').innerHTML += "<br><version style='font-size: 10px;'>Webster</version> &nbsp;" + webster[book][ch][i][0];
			//document.getElementById('greektext').innerHTML += "<br>";
		}
		
		if( g_eng_mlv_per_line == 1 )
		{	
			document.getElementById('greektext').innerHTML += "<br><version style='font-size: 12px;'>MLV</version> &nbsp;" + MLV[book][ch][i][0];
			//document.getElementById('greektext').innerHTML += "<br>";
		}
		
		if( g_eng_web_per_line == 1 || g_eng_mlv_per_line == 1 )
			document.getElementById('greektext').innerHTML += "<br><br>";
